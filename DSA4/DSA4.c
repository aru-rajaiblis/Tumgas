...
int* icecreamParlor(int m, int arr_count, int* arr, int* result_count) {
    // 1. Siapkan Brankas
    static int hash_table[10001];
    for(int i = 0; i < 10001; i++) hash_table[i] = 0;


    // 2. Mengalokasikan memori untuk jawaban
    int* result = (int*)malloc(2 * sizeof(int));
    *result_count = 2; 


    // 3. Mulai Pencaharian pasangan 
    for (int i = 0; i < arr_count; i++) {
        int current_price = arr[i];
        int complement = m - current_price;


        if (complement > 0 && complement <= 10000 && hash_table[complement] != 0) {
            result[0] = hash_table[complement]; 
            result[1] = i + 1;                  
            return result;
        }
